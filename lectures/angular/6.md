...
|---------|----------|------------------------|
| **1. Hook & Big Picture** | 15 min | Define Client-Server communication. Contrast local memory arrays with remote databases. |
| **2. Key Concepts & Core Ideas** | 15 min | Explain HttpClient async flows, CORS issues, RxJS piping, and error catch streams. |
| **3. Live Coding Walkthrough** | 45 min | Enable HttpClient providers, update services to request Django APIs, handle CORS, and manage HTTP failures. |
| **4. Check for Understanding** | 10 min | Q&A on Observable completion, CORS origin errors, and catching HTTP status codes. |
| **5. Class Wrap-up & Checkpoint** | 5 min | Verify games catalog fetches data dynamically from the running Django server. |
